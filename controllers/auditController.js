...
const auditFreezeController = {
    activateFreeze: async (req, res) => {
        try {
            ...
            return res.status(err.statusCode || 500).json({ status: false, error: err.message });
        }
    },
    ...
    deactivateFreeze: async (req, res) => {
        try {
            ...
            return res.status(200).json({ status: true, message: "Freeze deactivated successfully" });
        } catch (err) {
            return res.status(err.statusCode || 500).json({ status: false, error: err.message });
        }
    },
